...
ВАЖНО — ограничения фонового агента:
  1) Accessibility. launchd запускает osascript в СВОЁМ TCC-контексте (не терминала),
     ...
     кликнет/не напечатает, ПОКА не выдашь грант вручную:
       ...
       грант привязан к интерпретатору, а не к имени скрипта).
     ...
     В «Основные → Объекты входа и расширения» агент виден как citrix-vpn-watchdog.
...
main() {
  ...
  case "$cmd" in
    ...
    resume) cmd_resume ;;
    watch) exec "$SCRIPT_DIR/citrix-vpn-watchdog" ;;
    doctor) cmd_doctor ;;
